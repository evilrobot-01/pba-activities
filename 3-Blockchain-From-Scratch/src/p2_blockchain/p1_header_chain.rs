...
//! structure. We leared from the lecture that it is actually the headers that are hash linked, so
...
impl Header {
	...
	fn child(&self) -> Self {
		Self { height: self.height + 1, parent: hash(self), ..*self }
	}
	...
	fn verify_sub_chain(&self, chain: &[Header]) -> bool {
		...
		for header in chain.iter() {
			if header.parent != hash(parent) || header.height != parent.height + 1 {
				return false
			}
			parent = header;
		}
...
fn build_valid_chain_length_5() -> Vec<Header> {
	...
	for i in 0..4 {
		header = headers.last().unwrap().child();
		headers.push(header);
	}
	headers
}
...
fn build_an_invalid_chain() -> Vec<Header> {
	...
	for i in 0..5 {
		header = headers.last().unwrap().child();
		header.height = thread_rng().gen_range(1u64..100);
		headers.push(header);
	}
	headers
}
